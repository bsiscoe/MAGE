...
class GlitchPass extends Pass {
	 ...
	 *
	 * @param {Function} triggerFn - The trigger function for triggering the glitch effect.
	 * @param {number} [dt_size=64] - The size of the displacement texture
	 * for digital glitch squares.
	 */
	constructor( triggerFn, dt_size = 64 ) {

		super();
		
		/**
		 * The trigger function for triggering the glitch effect.
		 *
		 * @type {Function}
		 */
		...
		this.renderToScreen = false; // default to not rendering glitch effect until triggered by audio input
	}
	...
	render( renderer, writeBuffer, readBuffer /*, deltaTime, maskActive */ ) {
		...
		if (this.triggerFn && !this.triggerFn()) {
			this._clearTrigger(); // reset trigger to prevent glitch effect when trigger condition is not met
		} else {
			this._generateTrigger(); // generate new trigger when trigger condition is met to allow glitch effect to occur
		}

		if ( this._curF % this._randX == 0 && this.triggerFn() || this.goWild == true ) {
